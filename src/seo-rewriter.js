...
🔷 seo_title (SEO Title):
   ...
   - VIẾT HOA CHỮ ĐẦU MỖI TỪ (Title Case): "Dự Đoán" không phải "dự đoán"
   - Từ viết tắt LUÔN IN HOA TOÀN BỘ: XSMB, XSMN, XSMT, KQXS, XS, VL
...
// ── Title Case tiếng Việt (capitalize chữ đầu mỗi từ, uppercase từ viết tắt) ──
const ABBR_UPPERCASE = new Set([
  'xsmb', 'xsmn', 'xsmt', 'kqxs', 'xshcm', 'xsdng', 'xs', 'mb', 'mn', 'mt',
  'vl', 'vietlott', '3d', '3d+', '4d', 'max3d', 'max4d', 'mega645', 'power655',
]);

function toTitleCaseVI(str) {
  if (!str) return str;
  // Tách theo dấu phân cách "-" / "|" / "–" (giữ nguyên dấu)
  return str
    .split(/( - | – | \| |: )/)
    .map(segment => {
      ...
      return segment
        ...
        .map(word => {
          if (!word) return word;
          const lower = word.toLowerCase().replace(/[^a-z0-9đáàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵ]/gi, '');
          // Từ viết tắt → ALL CAPS
          if (ABBR_UPPERCASE.has(lower)) return word.toUpperCase();
          // Số, ngày tháng (16/04, 2024) → giữ nguyên
          if (/^\d/.test(word)) return word;
          // Capitalize chữ đầu (Unicode-safe cho tiếng Việt)
  ...
  if (jsonStart !== -1 && jsonEnd !== -1 && jsonEnd > jsonStart) {
    try {
      ...
      return {
        ...
        seo_title       : toTitleCaseVI((parsed.seo_title || '').trim()),
